...
class BilibiliService(BaseService):
    ...
    async def get_info(self, url: str, **kwargs) -> MediaMetadata | None:
        ...
        if not clean_info:
            raise BotError(
                ...
                message="No video info returned from Bilibili",
                url=url,
        ...
        if not formats:
            raise BotError(
                code=ErrorCode.METADATA_ERROR,
                message="No valid formats found for Bilibili",
                url=url,
                service=Services.BILIBILI,
                is_logged=True,
            )
        ...
        for video_format in formats:
            try:
                callback_data_video = BilibiliCallback(
                    type="video",
        ...
        if formats_data.get("best_audio"):
            try:
                callback_data_audio = BilibiliCallback(
                    type="audio",
    ...
    async def download_video(
        ...
        logger.info(f"Downloading Bilibili: {url} (format_selector={actual_format!r})")
        ...
        title = truncate_string(
            escape_html(clean_info.get("title") or "Bilibili") if clean_info else "Bilibili",
            100,
        )
        performer = (
            clean_info.get("uploader") or clean_info.get("channel") or "Bilibili"
        ) if clean_info else "Bilibili"
    ...
    async def download_audio(self, url: str, format_id: str | None) -> List[MediaContent]:
        ...
        job = await self.arq.enqueue_job(
            ...
            artist=clean_info.get("uploader", "Bilibili") if clean_info else "Bilibili",
